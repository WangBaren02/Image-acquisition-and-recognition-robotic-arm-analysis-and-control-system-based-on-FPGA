...
//摄像头接?
    ...
    output  wire            cam_pwdn 		,  //摄像头时钟?择信号
...
wire	[28:0]	ram_wr_data	;	//前十位是x 中间十位y 后九位角?
...
assign wr_data={ycbcr_y[7:3],ycbcr_y[7:2],ycbcr_y[7:3]};    //取亮度??
...
axi_ddr_top
#(
.DDR_WR_LEN(64),//写突发长? ??128?64bit
.DDR_RD_LEN(64)//读突发长? ??128?64bit
)
ddr_rw_inst(
  .ddr3_clk     (clk_320m       ),
  ...
  .wr_e_addr    (H_PIXEL*V_PIXEL*2  ), //写结束地?,8位一字节对应?个地??16位x2
  .wr_rst       (1'b0           ), //写地?复位 wr_rst
  //读用户接?
  .user_rd_clk  (clk_25m        ), //读时?
  .data_rden    (rd_en          ), //读使能，高电平有?
  .data_rd      (rd_data        ), //读数?16?
  .rd_b_addr    (30'd0          ), //读起始地?
  .rd_e_addr    (H_PIXEL*V_PIXEL*2  ), //写结束地?,8位一字节对应?个地?,16位x2
  .rd_rst       (1'b0           ), //读地?复位 rd_rst
  .read_enable  (1'b1           ),
